...
public class SemanticAnalyzer extends VisitorAdaptor {
	...
	public void visit(IdentEnumItem item) {
		...
		if (enumValues.contains(enumValueCounter + 1)) {
			report_error("Član enumeracije '" + name + "' ne moze da ima vrednost " + enumValueCounter + 1, item);
			return;
		}
	...
	public void visit(DesignatorFactor_e designatorFactor_e) {
		...
		if (desObj.getKind() == Obj.Type) {
			report_error("Nije moguce dodeljivanje tipa: " + desObj.getName(), designatorFactor_e);
			designatorFactor_e.struct = Tab.noType;
			return;
		}
	...
	public void visit(Term_array term_array) {
		...
		if (!((term.assignableTo(Tab.intType) || term.getKind() == Struct.Enum)
				&& (factor.assignableTo(Tab.intType) || factor.getKind() == Struct.Enum))) {
	...
	public void visit(TermList_array termList_array) {
		...
		if (!((tl.assignableTo(Tab.intType) || tl.getKind() == Struct.Enum)
				&& (tl_array.assignableTo(Tab.intType) || tl_array.getKind() == Struct.Enum))) {
			report_error("Operandi moraju biti celobrojni tipoviii", termList_array);
	...
	public void visit(ArrayDesignator arrayDesignator) {
		...
		Struct expr = arrayDesignator.getExpression().struct;
		if (arrObj == Tab.noObj) {
			arrayDesignator.obj = Tab.noObj;
		} else if (!(expr.compatibleWith(Tab.intType) || expr.getKind() == Struct.Enum)) {
	...
	@Override
	public void visit(DotDesignatorIdent dotDesignator) {
	    ...
	    if (desObj.getType().getKind() != Struct.Enum) {
	        report_error("Pristup članovima je dozvoljen samo za enumeracije", dotDesignator);
	        dotDesignator.obj = Tab.noObj;
	        return;
	    }

	    String enumMemberName = dotDesignator.getI2();
	    ...
	    if (enumVal == Tab.noObj) {
	        report_error("Enumeracija nema član: '" + enumMemberName + "'", dotDesignator);
	        dotDesignator.obj = Tab.noObj;
	        return;
	    }
	...
	@Override
	public void visit(DotDesignatorLength dotDesignatorLength) {
	    Obj desObj = dotDesignatorLength.getDesignator().obj;

	    if (desObj == Tab.noObj) {
	        report_error("Nevalidan designator", dotDesignatorLength);
	        dotDesignatorLength.obj = Tab.noObj;
	        return;
	    }

	    if (desObj.getType().getKind() != Struct.Array) {
	        report_error("Atribut '.length' je dostupan samo za nizove", dotDesignatorLength);
	        dotDesignatorLength.obj = Tab.noObj;
	        return;
	    }

	    dotDesignatorLength.obj = new Obj(Obj.Var, desObj.getName() + ".length", Tab.intType);
	    dotDesignatorLength.obj.setAdr(desObj.getAdr());
	    dotDesignatorLength.obj.setLevel(desObj.getLevel());
	}
	...
	public void visit(ExprDesignatorStatement exprDesignatorStatement) {
		...
		Obj desObj = exprDesignatorStatement.getDesignator().obj;
		if (desObj.getType() == Tab.noType || expr == Tab.noType)
			return;
		...
		Boolean isAssignable = expr.assignableTo(desObj.getType())
				|| (desObj.getType().getKind() == Struct.Enum && expr == Tab.intType);
		if (!isAssignable) {
	...
	public void visit(ForWithCondition forWithCondition) {
		Struct cond = forWithCondition.getCondition().struct;
		if (!cond.compatibleWith(boolType)) {
			report_error("Uslov u 'for' petlji mora biti logičkog tipa", forWithCondition);
			return;
		}
	}
	...
	public void visit(CaseLabel caseLabel) {
		...
		int num = caseLabel.getN1();
		if (switchCaseValues.contains(num)) {
			report_error("Vrednost case-a (" + num + ") nije jedinstvena u switch naredbi", caseLabel);
			return;
		}
	...
	public void visit(PrintExpression_num printExpression_num) {
		...
		if (expr == Tab.noType) {
			printExpression_num.struct = Tab.noType;
			return;
		}
		if (!(expr.compatibleWith(Tab.charType) || expr.compatibleWith(Tab.intType) || expr.compatibleWith(boolType))) {
	...
	public void visit(PrintExpression_non_num printExpression_non_num) {
		...
		if (expr == Tab.noType) {
			printExpression_non_num.struct = Tab.noType;
